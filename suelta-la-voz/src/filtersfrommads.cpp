...
void filterGlitch(IplImage * img,int width, int height,senseMaker * senses[])
{
	
	char * pixels = img->imageData;
	//char  pixelsTmp[] = pixels; 
	long counter[] = {0,0,0};
	...
	for(int pos = 0; x < width;x++)
		for(int y = 0; y < height; y ++)
		{
			for(int c = 0; c < 3;c++)
			{
				...
				pixels[counter[c]] = pixels[counter[c]];
			}
		}*/
	...
	/*for( int y=0; y<img->height; y++ ) { 
		...
		{
			int value = ptr[3*x] + ptr[3*x+1]+ ptr[3*x+2];
			if (value > 255)
			{
				ptr[3*x] = ptr[3*x+1];
			}
...
void filterFrameit(unsigned char * pixels,int width, int height, int amount){
	...
	int border = 10;
	cout << sizeof(pixels) / sizeof(char);
	cout << " hep\n";
