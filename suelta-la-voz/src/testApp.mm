...
void testApp::update()
{
    if(camera->imageUpdated){
        ...
        if (filtertype==0) {
            ...
            cvRGB=ofcvColorImg.getCvImage();
        } else if (filtertype==1) {
            ofcvColorImg.setFromPixels(cameraPixelsNoAlpha,camera->width,camera->height);
            //cvRGB=cvCreateImage(cvSize(width,height), IPL_DEPTH_8U, 3); maybe this isn't needed???
            now=dis(0,"");
            cvRGB=ofcvColorImg.getCvImage();
            now=dis(now,"filter step 1");
            filterCurves_2(cvRGB, ofcvColorImg.getWidth(), ofcvColorImg.getHeight(),senses); //amount should be affected by senses
            now=dis(now,"filter step 2");
            //Note the the function will alter cvRGB, and that this will also alter ofcvColorImg as intended            
        } else if (filtertype==2) {
            ofcvColorImg.setFromPixels(cameraPixelsNoAlpha,camera->width,camera->height);
            now=dis(0,"");
            cvRGB=ofcvColorImg.getCvImage();
            now=dis(now,"filter step 1");
            filterCurves_hsv(cvRGB, ofcvColorImg.getWidth(), ofcvColorImg.getHeight(),0.5); //amount should be affected by senses
            now=dis(now,"filter step 2");
            //Note the the function will alter cvRGB, and that this will also alter ofcvColorImg as intended            
            ...
            cvRGB=ofcvColorImg.getCvImage();
            now=dis(now,"filter step 1");
            filterVignette_2(cvRGB, ofcvColorImg.getWidth(), ofcvColorImg.getHeight(),-1);
            now=dis(now,"filter step 2");
            //Note the the function will alter cvRGB, and that this will also alter ofcvColorImg as intended            
            ...
            for(int i =0; i < 10;i++)
            {
                ...
                ant(cameraPixelsNoAlpha,ofRandom(0,ofcvColorImg.getWidth()), ofRandom(0,ofcvColorImg.getHeight()),ofRandom(0,2*PI),ofcvColorImg.getWidth(), ofcvColorImg.getHeight(), 8,0.5);
            }     
            ofcvColorImg.setFromPixels(cameraPixelsNoAlpha,camera->width,camera->height);
            cvRGB=ofcvColorImg.getCvImage();
        } else if (filtertype==5) {
/*            cvRGB=cvCreateImage(cvSize(width,height), IPL_DEPTH_8U, 3);
            filterFrameit(ofcvColorImg.getPixels() , ofcvColorImg.getWidth(), ofcvColorImg.getHeight(),-1);
            cvRGB=ofcvColorImg.getCvImage(); //very very strange. if portrait mode, original and not filtered image copied to cvRGB
            //but correct filtered image drawn by ofcvColorImg.draw. weird. maybe because of documented bug in getPixels ofxCVimage.cpp
            //try getCvROIImage...
  */          
            //new try
        ...
        /*
        {
            ...
            cvSmooth(cvRGB, cvImageTemp, CV_BLUR , value);
            now=dis(now,"slow: time to filter step 4");
            cvRGB=cvImageTemp;
            now=dis(now,"ultrafast: time to filter step 5");
            ofcvColorImg=cvRGB; //works, and seems to be safe
...
void testApp::draw()
{
    ...
    if (!phototaken) {
        ...
        if (ofcvColorImg.width>ofcvColorImg.height) {
            //rotated, landscape pic
            ofPushMatrix();
            ofRotate(90);
            ofTranslate(0, -ofGetWidth());
            drawHeight=ofGetWidth();
            drawWidth=ofcvColorImg.width*drawHeight/ofcvColorImg.height;
            ofcvColorImg.draw(0, 0,drawWidth,drawHeight);
            ofPopMatrix();

        } else {
            drawWidth=ofGetWidth();
            drawHeight=ofcvColorImg.height*drawWidth/ofcvColorImg.width;
            ofcvColorImg.draw(0, 0,drawWidth,drawHeight);            
        }
        photodrawn=true;
	...
	if(touch.id == 0 && touch.y > ofGetHeight()- 70 && isButtonDown) {
        ...
        if (touch.x<100) {
            ...
            if((ofGetElapsedTimeMillis()-imagetime)>taptreshold){ //too avoid double tap on use button
                ...
                /* xxxxxxxxxxxxxxx
                 ofcvColorImg bör göras svart här eller hellre sätt flagga så att svart bild visas. kan man göra en draw här i touchup eller bara i draw? tror man kan här
                 */
